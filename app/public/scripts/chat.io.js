(function($){
	...
	var NICK_MAX_LENGTH = 15,
		...
		// server information
		serverAddress = 'appchat.aws.af.cm',
	...
	function bindDOMEvents(){
		$('.chat-input input').on('keydown', function(e){
			socket.emit('typing', { nick: nickname, room: currentRoom });
	...
	function bindSocketEvents(){
		...
		socket.on('typing', function(data){
			console.log(data, 'client side typing');
			//also should check socket if sockets aren't the same
			//then add another typing so it shows multiple users typing at the same time
			//also need to check if user deleted message after typing... then delete typing because no value

			//if user is not the same as the last user typing
			//if(data.socket != socket){
				
			//}else{

			//}
			if($('li.cType').length == 0){
				$('<li class="cType">' + data.nick + ' is typing...</li>').appendTo('.chat-messages ul');
			}
		});
