...
io.sockets.on('connection', function(socket){
	...
	socket.on('typing', function(data){
		updateTyping(socket, data.nick, data.room)
	});
...
function chatmessage(socket, data){
	...
	// the sender himself
	//updateTyping(data.room, chatClients[socket.id], true);
...
function updateTyping(socket, nickname, room){
	room = room.replace('/', '');
	//include socket to check for multiple users typing
	//socket.broadcast.to(room).emit('typing', { sock: socket, nick: nickname: room: room});
	socket.broadcast.to(room).emit('typing', { nick: nickname, room: room });
}
// unique id generator
function generateId(){
	var S4 = function () {
...
console.log('Chat server is running and listening to port %d...', process.env.VCAP_APP_PORT || 3000);